...
public class MessageThreadServiceImpl implements MessageThreadService {
    ...
    @Override
    public ThreadResponse createMessageThread(ThreadRequest.Create request) {
    ...
    public List<ThreadResponse> findAllMessageThreads(UUID courseUuid, ThreadStatus status, ThreadPriority priority, Boolean escalated, String search) {
    ...
    @Override
    @Transactional(readOnly = true)
    public ThreadResponse findMessageThreadByUuid(UUID threadUuid) {
        return ThreadResponse.from(findMessageThreadEntityByUuid(threadUuid));
    }

    @Override
    public ThreadResponse updateMessageThreadStatus(UUID threadUuid, ThreadRequest.UpdateStatus request) {
        var thread = findMessageThreadEntityByUuid(threadUuid);
        thread.setStatus(request.status());
        return ThreadResponse.from(threadRepository.save(thread));
    }

    @Override
    public ThreadResponse escalateMessageThread(UUID threadUuid, UUID escalatedByUuid) {
        var thread = findMessageThreadEntityByUuid(threadUuid);
    ...
    @Override
    public MessageThread findMessageThreadEntityByUuid(UUID uuid) {
