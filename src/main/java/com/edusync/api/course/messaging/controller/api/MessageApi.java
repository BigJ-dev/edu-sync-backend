...
@Tag(name = "Messages", description = "Endpoints for managing messages within a thread.")
@RequestMapping("/threads/{threadUuid}/messages")
public interface MessageApi {
    ...
    @Operation(
            ...
            description = "Adds a new message to a thread. Updates the thread's last message timestamp " +
    ...
    MessageResponse create(@PathVariable UUID threadUuid, @Valid @RequestBody MessageRequest.Create request);

    @GetMapping
    @Operation(
            summary = "List messages in a thread",
            description = "Returns all messages in a thread ordered by creation time ascending."
    )
    @ApiResponse(responseCode = "200", description = "Messages retrieved successfully")
    @ApiResponse(responseCode = "404", description = "Thread not found")
    List<MessageResponse> findByThread(@PathVariable UUID threadUuid);

    @PostMapping("/read")
    @Operation(
            summary = "Mark thread as read",
            description = "Marks all messages in a thread as read for a given reader. " +
