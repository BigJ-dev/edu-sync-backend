...
public class AttemptService {
    ...
    public AnswerResponse submitAnswer(UUID attemptUuid, AttemptRequest.SubmitAnswer request) {
        var attempt = findAttempt(attemptUuid);

        if (attempt.getStatus() != AttemptStatus.IN_PROGRESS) {
            throw new ServiceException(HttpStatus.BAD_REQUEST, "Cannot submit answers to a completed or timed out attempt");
        }

        var question = questionService.findQuestion(request.questionUuid());
    ...
    public AttemptResponse completeAttempt(UUID attemptUuid) {
        var attempt = findAttempt(attemptUuid);
    ...
    public AttemptResponse findByUuid(UUID attemptUuid) {
        return AttemptResponse.from(findAttempt(attemptUuid));
    }

    @Transactional(readOnly = true)
    public List<AnswerResponse> findAnswers(UUID attemptUuid) {
        var attempt = findAttempt(attemptUuid);
        var answers = answerRepository.findByAttemptId(attempt.getId());
    ...
    @Transactional(readOnly = true)
    public List<AttemptResponse> findAttemptsByQuizAndStudent(UUID quizUuid, UUID studentUuid) {
        var quiz = quizService.findQuiz(quizUuid);
    ...
    public QuizAttempt findAttempt(UUID uuid) {
