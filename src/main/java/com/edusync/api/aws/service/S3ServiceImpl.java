...
public class S3ServiceImpl implements S3Service {
    ...
    private Duration getPresignedTimeToLive() {
        var minutes = Long.parseLong(settingService.getSystemSettingValue("presigned_url_ttl_minutes"));
        return Duration.ofMinutes(minutes);
    }

    private long getMaxFileSize() {
        return Long.parseLong(settingService.getSystemSettingValue("max_file_upload_bytes"));
    }

    private Set<String> getAllowedMimeTypes() {
        var raw = settingService.getSystemSettingValue("allowed_upload_mime_types");
